...
TURSO_TOKEN = "eyJhbGciOiJFZERTQSIsInR5cCI6IkpXVCJ9.eyJhIjoicnciLCJpYXQiOjE3NzcwMzQyNTcsImlkIjoiMDE5ZGJlYTEtM2EwMS03MmU2LWE2ZDgtN2ZmM2RkYzVlMTZhIiwicmlkIjoiY2JiOTcwYWUtYmQxOC00MjYzLTgzYzgtMTU4NDk0OGNiODk4In0.peBfaXt2cRPb6ZPRqbDwPpiqsBGVvL5X1-qeD5nuER0gR_Wwx9metptpIilE2UfyeWD_2zwsTL-V1Q-JAJ1zDw"
...
def edit_article():
    ...
    conn = connecter_sqlite()  # libsql_experimental.connect(...)
    try:
        ...
        if nouvelle_reference != reference_originale:
            result_conflict = conn.execute(
                "SELECT reference FROM correspondance_article WHERE reference = ?",
                (nouvelle_reference,)
            )
            conflict = result_conflict.fetchone()
            if conflict:
